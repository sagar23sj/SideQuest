...
class Converters {
    ...
    @TypeConverter
    fun fromTaskReminder(value: TaskReminder?): String? =
        value?.let { RoomJson.encodeToString(TaskReminder.serializer(), it) }

    @TypeConverter
    fun toTaskReminder(value: String?): TaskReminder? =
        value?.let { RoomJson.decodeFromString(TaskReminder.serializer(), it) }
